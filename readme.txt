...
* Fixed: inline `style` attributes are no longer stripped from `<input>` elements in the Hidden Fields box, and CSS declarations used to visually hide tracking pixels and honeypots (`display`, `visibility`, `opacity`, `position`, `top`/`right`/`bottom`/`left`, `z-index`, `pointer-events`) now survive sanitization. This restores tracking pixels and visually-hidden honeypot fields used by Flodesk and similar services. Props 17thavenue for the report (issue 164).
